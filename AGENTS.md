...
- **`--velpari-fix` flag** (v1.4.0). Registered in `pi-extension/src/index.ts` alongside the two flags above. When set, every `/velpari-doctor` invocation follows up with an interactive 2-level picker (`pi-extension/src/ui/fix-picker.ts`) that shows actionable items; selecting one dispatches the parent LLM via `pi.sendUserMessage` (`pi-extension/src/doctor/fix-dispatch.ts`). **Default off** — opt-in per the conservative-fix principle. The doctor never edits `Doc/` directly; every fix routes through an existing `/velpari-*` slash command, so the `tool_call` mutation lock still gates every write. The picker lives at L2 and the orchestrator at L3 (`commands/doctor.ts`); `doctor/` (L1) cannot import `ui/` (L2) under the layer rule, so `handleDoctor` returns `HandleDoctorResult { skipped, report }` and the orchestration reads the report back. Future: Phase 2 (v1.5.x) turns the `"all-safe"` picker branch into declarative auto-remediation; Phase 3 (v1.6.x) replaces the structured prompt with a `FixBrief` for agentic fix via the parent LLM. Full design in `.IDE_Plans/doctor-fix-upgrade_plan_20260919_1318_v1.0.md`.
