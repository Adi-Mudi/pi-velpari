...
10f. **DB-primary reads (Phase 6) + DB-auditing doctor (Phase 7).** The per-project SQLite store (`Doc/store/<project>/index.db`) is the single machine source of truth; markdown/YAML/HTML are human views (DB-rendered for 5 artifacts — prd, rtm, atomic-functions, test-cases, development-order; LLM-authored + hash-bound for 5 hybrid docs — design, pseudocode, testplan-plan, feasibility-study, final-design). Stages + scouts read `## DB Input Slices` blocks from the store only; brainstorm notes remain the one file-based input; the reviewer agent is the sole slice+view exception (drift = a finding). Sidecar YAML files are retired as sources (legacy read fallback — kept because pre-store / flag-ON projects and the migration loaders still rely on it; removal is **deliberately deferred**, not scheduled: locked decision OQ-P12c→(a) took both removals — the sidecar fallback and the legacy `Doc/` markdown — out of Phase 12, and each needs its own plan + delete gates) — YAML is a download view via `/velpari-export`. A pre-store project recovers with the one-step `/velpari-backfill <kind>` import (43rd command; idempotent, store-only — no Doc/ write, no stage advance, no git commit). Phase 7: the doctor's data checks read store rows directly (sidecar fallback + a `/velpari-backfill` warning for pre-store projects); new SQL checks audit the DB itself — G4 integrity (`PRAGMA quick_check` + `integrity_check`, stamping `store_meta.integrity_checked_at` on standalone `/velpari-doctor` runs only, never in the embedded post-publish run) and the `links` adjacency orphan audit; the secrets scan sweeps DB text columns; id-coverage uses store link edges (tc_trace/step_af) with sidecar fallback.

10g. **DB-only publish + one-time migration (Phase 11, Q3/RES-3).** Publish is **DB-only by default**: approve writes store rows + the YAML export + a git commit and NOTHING to `Doc/` — the `.IDE_Plans/velpari/runs/<run-id>/<stage>/*.md` working copies stay the temp review surface, existing `Doc/` markdown stays as readable history, and views come from `/velpari-export` + the `show` commands. Write-alongside is the explicit opt-IN rollback hatch: `files.json` `"velpari": {"markdownWrites": true}` (`core/config.ts:markdownWritesEnabled`, absent = OFF; `skipDbPublish` implies ON for the markdown-only test mode). `/velpari-migrate-store` (45th command, `ops/migrate.ts`) is the one-time RES-3 import: `--dry-run` writes nothing; `--execute` confirms first, then per project migrates kinds in FK order under run id `migrated`, re-exports all 9 YAMLs beside the DB, verifies checksums/integrity/links/registry, and commits explicit paths only (`velpari(migrate): <project> (run migrated)`). G7: an in-flight run hard-blocks (message names `/velpari-reset`); the publish git-identity precheck runs before any write. Retirement blast radius is covered: DB-era freshness inputs hash the exported YAML bytes, handoff renders design from the store (file = fallback), reconfirm appends its audit line to the envelope `changeLog` column when no published file exists.
...
> **Phase 11 (Q3):** publish is DB-only by default — those `Doc/` paths are the store kinds' human view and are written only for legacy projects or with the `velpari.markdownWrites` opt-in (`files.json`). The store DB under `Doc/store/<projectName>/index.db` (+ its exported YAML beside it) is the source; legacy `Doc/` markdown stays on disk as readable history.
...
**45 commands total** (see `commands/index.ts:COMMAND_NAMES` for the full list):
...
- **9 ops/discipline** — `status`, `reset`, `handoff`, `doctor`, `design-logging`, `reconfirm` (A5), `backfill` (Phase 6 — 43rd command), `portfolio` (Phase 10 — 44th command), `migrate-store` (Phase 11 — 45th command; DB-only publish + one-time migration, Q3/RES-3).
