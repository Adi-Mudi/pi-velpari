...
- **Store DB is git-committed raw; the publish chain auto-heals the user repo's `.gitattributes`/`.gitignore`** (ops/git-attributes.ts — binary attr + WAL ignores; healed files join the publish commit) and the merge/recovery runbook ships at `skills/db-store-merge-runbook.md` (rebuild path: `/velpari-backfill <kind> --from-export`). Phase 9, §15.4.
