...
published test plan and test cases in place — never rewrite them from
scratch. Explicitly name the upstream changes (from the revised
pseudocode and PRD) that triggered this revision.

Revision rules:

1. **Append-only TC IDs.** New FRs from the revised PRD get new TC-NN /
   IT-NN IDs appended at the next free number. Never renumber or reuse
   an existing ID.
2. **Deprecate, don't delete.** Test cases for FRs deprecated in the PRD
   keep their rows marked `deprecated` with a reason. Never delete the
   row.
