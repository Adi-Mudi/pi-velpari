...
describe("financial-payments overlay — catalogue", () => {
	it("is listed in the bundled catalogue", () => {
		...
		assert.deepStrictEqual(entry!.standards, ["PCI-DSS v4.0", "SOX §404", "FFIEC CAT v1.0", "ISO 27001:2022"]);
	});
...
describe("financial-payments overlay — profile", () => {
	...
	it("declares 8 doctor checks", () => {
		...
		assert.ok(overlay.doctorChecks.some((c) => c.includes("Cardholder Data Environment") || c.includes("CDE")));
		assert.ok(overlay.doctorChecks.some((c) => c.toLowerCase().includes("segregation of duties")));
	});
});
