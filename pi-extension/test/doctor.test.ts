...
/** Phase 1: runDoctor returns a DiagnosticReport; runText() formats it
 *  for the legacy substring assertions below. New structural assertions
 *  use runDoctor directly. */
function runText(cwd: string): string {
	return formatDiagnosticReport(runDoctor(cwd));
}
...
// ---------------------------------------------------------------------------
// runDoctor: structured shape (Phase 1)
// ---------------------------------------------------------------------------

test("runDoctor returns a DiagnosticReport with sections, summary, and verdict", () => {
	const dir = tempDir();
	try {
		...
		assert.ok(report.sections.length >= 10, "expected at least 10 sections");
	} finally {
		rmSync(dir, { recursive: true, force: true });
	}
});

test("runDoctor summary totals match item counts across all sections", () => {
	const dir = tempDir();
	try {
		const report = runDoctor(dir);
		const total = report.summary.ok + report.summary.warning + report.summary.error + report.summary.info;
		let itemCount = 0;
		for (const section of report.sections) itemCount += section.items.length;
		assert.equal(total, itemCount);
	} finally {
		rmSync(dir, { recursive: true, force: true });
	}
});

test("runDoctor verdict is false when any item is an error", () => {
	const dir = tempDir();
	try {
		// Inject a scout agent with bad frontmatter to trigger an error.
		const agentsDir = join(dir, ".pi", "agents");
		mkdirSync(agentsDir, { recursive: true });
		writeFileSync(join(agentsDir, "extractor.md"), "---\nname: extractor\n---\nbody\n", "utf8");
		const report = runDoctor(dir);
...
// ---------------------------------------------------------------------------
// runDoctor new sections (Phase 1 — now via formatted text)
// ---------------------------------------------------------------------------

test("runDoctor includes the Multiplexer section", () => {
	const dir = tempDir();
	const report = runText(dir);
	assert.match(report, /## Multiplexer \(required for \/velpari-discuss v2\.0\)/);
	assert.match(report, /Multiplexer detected:/);
...
test("runDoctor warns when no multiplexer is detected", () => {
	...
	try {
		const report = runText(dir);
		assert.match(report, /Multiplexer detected: unknown/);
		assert.match(report, /Start pi inside tmux, zellij, wezterm, or cmux/);
	} finally {
...
test("runDoctor includes zellij Issue #19 warning when ZELLIJ_PANE_ID is set", () => {
	...
	try {
		const report = runText(dir);
		assert.match(report, /Multiplexer detected: zellij/);
...
test("runDoctor includes Scout agents section", () => {
	...
	assert.match(report, /extractor\.md \(discuss\) MISSING/);
	assert.match(report, /prd-checker\.md \(discuss\) MISSING/);
	assert.match(report, /rtm-checker\.md \(discuss\) MISSING/);
	assert.match(report, /web-search-agent\.md \(discuss\) MISSING/);
	rmSync(dir, { recursive: true, force: true });
});
...
test("runDoctor warns when .pi/agents/extractor.md has bad frontmatter", () => {
	...
	const report = runText(dir);
	assert.match(report, /extractor\.md \(discuss\) frontmatter missing/);
	rmSync(dir, { recursive: true, force: true });
});
...
test("runDoctor enumerates Scout agents for all 9 stages (Phase 6)", () => {
	...
	const scoutSection = report.sections.find((s) => s.title === "Scout agents (.pi/agents/)");
	assert.ok(scoutSection, "scout agents section must exist");
	const text = formatDiagnosticReport({ ok: true, summary: { ok: 0, warning: 0, error: 0, info: 0 }, sections: [scoutSection] });
	for (const stage of [
		"discuss",
		"prd",
		"rtm",
		...
		// Match `(stage)` (parens appear in MISSING/bad-frontmatter items
		// like `extractor.md (discuss) MISSING`) OR `stage,` (comma in the
		// summary's stage list).
		const re = new RegExp(`(?:\\(${stage.replace(/[-]/g, "-")}\\)|${stage.replace(/[-]/g, "-")},)`);
		assert.match(text, re, `Scout agents section should list ${stage}`);
	}
});
...
test("runDoctor checks Stage skills for all 10 stages (Phase 7: adds configure-requirements)", () => {
	const cwd = resolve(__dirname, "..", "..", "..");
	const report = runDoctor(cwd);
	const stageSkillsSection = report.sections.find((s) => s.title === "Stage skills");
	assert.ok(stageSkillsSection, "stage skills section must exist");
	const text = formatDiagnosticReport({ ok: true, summary: { ok: 0, warning: 0, error: 0, info: 0 }, sections: [stageSkillsSection] });
	for (const stage of [
		"discuss",
		"prd",
		"rtm",
		...
		assert.ok(text.includes(`skills/velpari-${stage}.md`), `Stage skills should reference skills/velpari-${stage}.md`);
	}
});
...
test("runDoctor Scout agents summary counts all 36 scouts (9 stages × 4)", () => {
	const cwd = resolve(__dirname, "..", "..", "..");
	const report = runText(cwd);
	assert.match(report, /36 scouts across 9 stages/);
});

test("runDoctor Stage skills summary covers all 10 skills (Phase 7)", () => {
	const cwd = resolve(__dirname, "..", "..", "..");
	const report = runText(cwd);
	assert.match(report, /10 checked \(discuss, prd, rtm, feasibility, design, pseudocode, testplan, atomic-function, development-order, configure-requirements\)/);
});
...
test("handleDoctor emits a summary notify and writes the formatted report", async () => {
	...
	try {
		...
		// First notify is the summary line; it must mention "Doctor:".
		assert.match(notifyCalls[0]!.msg, /Doctor:/);
		// The full report file must be written.
		const reportPath = join(dir, ".IDE_Plans", "velpari", "doctor-report.md");
		assert.ok(existsSync(reportPath), "doctor report file must exist");
	} finally {
		rmSync(dir, { recursive: true, force: true });
	}
});
