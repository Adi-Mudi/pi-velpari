...
function setupValidRun(dir: string, projectName = "TestApp", mission = "Mission"): void {
	...
	// Write all 7 expected input artifacts.
	const slug = "mission";
	writeFileSync(join(dir, "Doc", `discussion-${slug}.md`), "# stub discussion\n", "utf8");
	...
	writeFileSync(join(dir, "Doc", `test-cases_${projectName}.md`), "# stub test cases\n", "utf8");
}

// Gate checks

test("handleAtomicFunction refuses when no active run exists", async () => {
	const dir = tempDir();
	try {
		saveFilesConfig(
...
test("handleAtomicFunction refuses when projectName is missing", async () => {
	const dir = tempDir();
	try {
		createRun("Mission", dir);
		const ui = makeMockUI();
...
test("handleAtomicFunction refuses when one of the 7 published artifacts is missing", async () => {
	const dir = tempDir();
	try {
		saveFilesConfig(
		...
		createRun("Mission", dir);
		mkdirSync(join(dir, "Doc"), { recursive: true });
		// Only write 6 of the 7 artifacts.
		const slug = "mission";
		...
		writeFileSync(join(dir, "Doc", "pseudocode_TestApp.md"), "# stub\n", "utf8");
		// Missing: test-plan_TestApp.md and test-cases_TestApp.md
		const ui = makeMockUI();
		const pi = makeMockPi();
		const ctx = { ui, cwd: dir } as never;
		await handleAtomicFunction(ctx, pi as never, dir);
...
test("handleAtomicFunction calls pi.sendUserMessage with the assembled prompt", async () => {
	const dir = tempDir();
	try {
		setupValidRun(dir);
		const ui = makeMockUI();
...
test("handleAtomicFunction embeds all 4 af scout paths", async () => {
	const dir = tempDir();
	try {
		setupValidRun(dir);
		const ui = makeMockUI();
		...
		for (const s of ["af-source-rtm", "af-source-pseudocode", "af-source-prd", "af-source-testcases"]) {
...
test("handleAtomicFunction concatenates all 7 published artifacts into the prompt", async () => {
	const dir = tempDir();
	try {
		...
		await handleAtomicFunction(ctx, pi as never, dir);
		const prompt = pi.sent[0]!.prompt;
		// Each artifact's content should be present in the prompt
		assert.match(prompt, /stub discussion/);
		...
		assert.match(prompt, /stub pseudocode/);
		assert.match(prompt, /stub test plan/);
		assert.match(prompt, /stub test cases/);
	} finally {
		clearRun(dir);
		rmSync(dir, { recursive: true, force: true });
	}
...
test("handleAtomicFunction does NOT write the working copy (LLM's job)", async () => {
	const dir = tempDir();
	try {
		setupValidRun(dir);
		const ui = makeMockUI();
		...
		const wc = join(dir, ".IDE_Plans", "velpari", "runs", state.runId, "atomic-function", "atomic-functions_TestApp.md");
...
test("handleAtomicFunction does NOT mutate state.stage (optional stage)", async () => {
	const dir = tempDir();
	try {
		setupValidRun(dir);
		const ui = makeMockUI();
...
test("handleAtomicFunction bootstraps af agent files on first use", async () => {
	const dir = tempDir();
	try {
		setupValidRun(dir);
		const ui = makeMockUI();
		...
		for (const id of ["af-source-rtm", "af-source-pseudocode", "af-source-prd", "af-source-testcases"]) {
...
test("handleAtomicFunction preserves unicode mission verbatim in prompt", async () => {
	const dir = tempDir();
	try {
		...
		if (!existsSync(discussionPath)) {
			writeFileSync(discussionPath, "# stub\n", "utf8");
		}
		const ui = makeMockUI();
		const pi = makeMockPi();
		const ctx = { ui, cwd: dir } as never;
...
test("handleAtomicFunction handles very long mission (>2KB) without error", async () => {
	const dir = tempDir();
	try {
...
test("handleAtomicFunction works when framework is undefined (omits Framework line)", async () => {
	const dir = tempDir();
	try {
		saveFilesConfig(
		...
		writeFileSync(join(dir, "Doc", "test-cases_X.md"), "# stub\n", "utf8");
		const ui = makeMockUI();
		const pi = makeMockPi();
		const ctx = { ui, cwd: dir } as never;
		await handleAtomicFunction(ctx, pi as never, dir);
...
test("handleAtomicFunction refuses to write outside the run dir when mission has path-traversal characters", async () => {
	const dir = tempDir();
	try {
		...
		await handleAtomicFunction(ctx, pi as never, dir);
		const state = loadState(dir);
		const wc = join(dir, ".IDE_Plans", "velpari", "runs", state.runId, "atomic-function", "atomic-functions_X.md");
		assert.equal(existsSync(wc), false);
	} finally {
		clearRun(dir);
		rmSync(dir, { recursive: true, force: true });
...
test("handleAtomicFunction does not crash when the atomic-function output dir already has stale content", async () => {
	const dir = tempDir();
	try {
		setupValidRun(dir);
		const state = loadState(dir);
		const atomicDir = join(dir, ".IDE_Plans", "velpari", "runs", state.runId, "atomic-function");
		mkdirSync(atomicDir, { recursive: true });
		writeFileSync(join(atomicDir, "stale.txt"), "stale", "utf8");
		const ui = makeMockUI();
		const pi = makeMockPi();
		const ctx = { ui, cwd: dir } as never;
		await handleAtomicFunction(ctx, pi as never, dir);
		assert.equal(pi.sent.length, 1);
		assert.ok(existsSync(join(atomicDir, "stale.txt")));
		assert.ok(existsSync(join(atomicDir, "scouts")));
	} finally {
		clearRun(dir);
		rmSync(dir, { recursive: true, force: true });
	}
...
test("handleAtomicFunction output working-copy path uses atomic-functions_<project>.md", async () => {
	const dir = tempDir();
	try {
		setupValidRun(dir, "TestApp");
		const ui = makeMockUI();
		...
		await handleAtomicFunction(ctx, pi as never, dir);
		const prompt = pi.sent[0]!.prompt;
		assert.match(prompt, /atomic-functions_TestApp\.md/);
	} finally {
		clearRun(dir);
		rmSync(dir, { recursive: true, force: true });
	}
