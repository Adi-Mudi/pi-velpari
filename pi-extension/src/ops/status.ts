...
// Local copy kept here so the L1 ops layer never imports from L2 ui/.
...
export async function handleStatus(
	...
	if (projectName) {
		...
		const groupedBrainstorm = resolveBrainstormArtifact(topicSlug, cwd);
		if (groupedBrainstorm) {
			lines.push(`- Brainstorm: ${groupedBrainstorm.path} (${groupedBrainstorm.layout})`);
		} else {
			lines.push(`- Brainstorm: (none at topic slug "${topicSlug}")`);
		}
	} else {
	...
	if (existsSync(docDir)) {
		...
		// Also note legacy brainstorm files explicitly.
		const legacyBrainstorm = flat.filter((f) => f.startsWith("brainstorm-"));
		if (legacyBrainstorm.length > 0) {
			lines.push(``, `## Legacy flat brainstorms`);
			for (const f of legacyBrainstorm) {
				lines.push(`- ${f}`);
			}
		}
		void groupedSet;
		void groupedBrainstormPrefix;
	}
