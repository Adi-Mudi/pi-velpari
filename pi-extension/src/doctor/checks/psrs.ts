...
import { findFrRowsMissingKeywords, renderPsrsSummary, validatePsrs } from "../../core/psrs.js";
...
export function checkPsrsSection(cwd: string, projectName: string): DiagnosticSection {
	...
	if (existsSync(grouped)) {
		...
		// Phase 4 (RTM traceability upgrade): RFC 2119 wording — warn on
		// FR rows whose requirement text has no MUST/SHALL/SHOULD/MAY.
		const missingKeywords = findFrRowsMissingKeywords(content);
		if (missingKeywords.length > 0) {
			items.push({
				status: "warning",
				message: `${missingKeywords.length} FR row(s) carry no RFC 2119 keyword (MUST/SHALL/SHOULD/MAY): ${missingKeywords.join(", ")}.`,
				suggestion: suggestionFor("fr-wording"),
			});
		}
		return { title: "PSRS validation", items };
	}
	...
	items.push({
		status: "info",
		message: "PSRS not found yet. It will be created when you run `/velpari-prd` after `/velpari-brainstorm`.",
