...
if (!p.pi || !Array.isArray(p.pi.extensions) || p.pi.extensions.length === 0) {
  ...
  ok = false;
    ...
    !p.pi.extensions[0].endsWith('dist/index.js') &&
    !p.pi.extensions[0].endsWith('dist/pi-extension/src/index.js') &&
    !p.pi.extensions[0].endsWith('index.js') &&
    !p.pi.extensions[0].endsWith('index.ts')
  ) {
  console.error('FAIL: pi.extensions entry must end in index.js or index.ts: ' + p.pi.extensions[0]);
  ok = false;
}
if (p.type !== 'module') {
