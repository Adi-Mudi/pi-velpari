...
> **v0.5.0 architecture update (2026-09-05):** FR-54 and FR-56 still describe the v1.5 `ScoutContract` interface (a v1.5 file no longer in the v0.4.0 codebase) for historical traceability. The v1.5 contract was removed in v2.0; today the contract lives per-stage in the per-stage skill markdown (`skills/velpari-*.md`). The v1.5 module was removed in v2.0; v0.4.0 has no such file. The v0.5.0 source layout replaced the v1.5 file structure; see `Doc/design.md` §v0.4.0 for the full path map. This callout is the allowlist anchor for the doc-hygiene regression test (the only place in this file where `contracts.ts` may appear).
...
| **FR-54** | **All 36 scout agents follow the same `stages/registry.ts:runStageWithScouts` orchestration** (4 scouts per stage; same JSON output envelope `{ proposals: [...], source: ScoutId }`; same 30-second timeout; same picker UI). The original v1.5 `ScoutContract` TypeScript interface was removed in v2.0; the contract is now defined per-stage in the per-stage skill markdown (`skills/velpari-*.md`). |
...
| **FR-56** | **`ScoutContract` interface (v1.5, removed in v2.0; contract now lives in per-stage skill markdowns)**. Scout files live at `skills/scouts/{scoutId}.md` (legacy) / `skills/velpari-*.md` (current). |
...
| **NFR-13** | Uniform subagent pattern | All 12 scout agents (3 in discuss + 4 AF-SCOUT + 4 DO-SCOUT + 1 optional WEB SEARCH) follow the same `ScoutContract` (per the v1.5 `ScoutContract`, removed in v2.0; contract now lives in per-stage skill markdowns). Same spawn helper, same JSON output envelope, same 30-second timeout, same picker UI. Drift between any two scouts is a defect caught by tests TC-189..TC-198. |
...
- **v1.5** — added FR-49..FR-57 (framework as one-time setup in `/velpari-configure-inputs`; discussion stage 4 agents NEW EXTRACTOR + PRD CHECKER + RTM CHECKER + WEB SEARCH AGENT replacing DECISION AGENT; DECISION logic moved to main handler; WEB SEARCH is user-prompted and collects community + official docs + similar projects; all 12 scouts follow `ScoutContract`; no runtime dependency on Senai; `ScoutContract` defined in v1.5 (removed in v2.0); `framework` field validated). Added NFR-13 (uniform subagent pattern). Added constraints: no runtime dependency on Senai (TUI re-implemented); framework is one-time setup, not a pipeline stage.
